...
public class FaceServer {
    ...
    private void initFaceList(Context context) {
        ...
        synchronized (this) {
            ...
            //特征存储文件夹，root/register/features
            ...
            //得到的是一个 File 类型的数组，返回的是该目录中的文件和目录
            ...
            for (File featureFile : featureFiles) {
                try {
                    ...
                    //将最多 feature.length 个字节的数据读入一个 feature 数组中
                    int a=fis.read(feature);
                    System.out.println("从文件输入流中读取了"+a+"个字节的数据");
                    fis.close();
    ...
    public int getFaceNumber(Context context) {
        synchronized (this) {
            ...
            Log.i(TAG, ROOT_PATH + File.separator + SAVE_IMG_DIR);
    ...
    public boolean registerBgr24(Context context, byte[] bgr24, int width, int height, String name) {
        synchronized (this) {
            ...
            Log.e(TAG, "特征存储的路径"+ROOT_PATH + File.separator + SAVE_FEATURE_DIR);
            ...
            //图片存储的文件夹
            File imgDir = new File(ROOT_PATH + File.separator + SAVE_IMG_DIR);
            Log.e(TAG, "图片存储的文件路径"+ROOT_PATH + File.separator + SAVE_IMG_DIR);
            if (!imgDir.exists() && !imgDir.mkdirs()) {
            ...
            //人脸检测
            List<FaceInfo> faceInfoList = new ArrayList<>();//检测到的人脸信息
     ...
     * @return faceFeature
     */
    public byte[] registerFaceFeature(Context context, byte[] bgr24, int width, int height, String name) {
        byte[] faceFeatureData=new byte[1032];
        synchronized (this) {
            if (faceEngine == null || context == null || bgr24 == null || width % 4 != 0 || bgr24.length != width * height * 3) {
                Log.e(TAG, "registerBgr24:  invalid params");
                return null;
